...
import com.gogofnd.theCreditableCMS.domain.basic.dto.BasicListDto;
...
import static com.gogofnd.theCreditableCMS.domain.basic.entity.QBasicEntity.basicEntity;
...
//        if(keyword.equals("")) return null;
        return basicEntity.companyName.contains(keyword)
                .or(basicEntity.companyContent.contains(keyword))
                .or(basicEntity.companyNum.stringValue().contains(keyword));
    }
    ...
    public Boolean exist(QueryModel queryModel) {
        Integer fetchOne = queryFactory
                .selectOne()
                .from(basicEntity)
                .where(basicEntity.companyNum.stringValue().contains(queryModel.getKeyword()))
                .fetchFirst();
    ...
    public Boolean existName(QueryModel queryModel) {
        Integer fetchOne = queryFactory
                .selectOne()
                .from(basicEntity)
                .where(basicEntity.companyName.contains(queryModel.getKeyword()))
                .fetchFirst();
    ...
    public Boolean existContent(QueryModel queryModel) {
        Integer fetchOne = queryFactory
                .selectOne()
                .from(basicEntity)
                .where(basicEntity.companyContent.contains(queryModel.getKeyword()))
                .fetchFirst();
    ...
    private BooleanExpression keywordColumnName(QueryModel queryModel){
        if(Objects.equals(queryModel.getColumnName(), "companyName")) {
            return basicEntity.companyName.contains(queryModel.getKeyword());
        } else if (Objects.equals(queryModel.getColumnName(), "companyContent")) {
            return basicEntity.companyContent.contains(queryModel.getKeyword());
        } else if (Objects.equals(queryModel.getColumnName(), "companyNum")) {
            return basicEntity.companyNum.stringValue().contains(queryModel.getKeyword());
        }
        return null;
    }
    ...
    private BooleanExpression betweenDate(QueryModel queryModel){
        if(queryModel.getStartDate() == null || queryModel.getEndDate() == null) {
            ...
            return  basicEntity.signUpDateTime.between(queryModel.getStartDate(),endDate);
        }
    }
    ...
    public Page<BasicListDto> findAllCompany(Pageable pageable, QueryModel queryModel) {
        List<BasicListDto> resultBasicListDto =
                queryFactory.select(Projections.constructor(BasicListDto.class, basicEntity))
                        .from(basicEntity)
                        .where(basicEntity.useYN.eq("Y")
                                ,betweenDate(queryModel))
                        .offset(pageable.getOffset())
                        .limit(pageable.getPageSize())
                        .orderBy(basicEntity.companyId.desc())
                        .fetch();
        int totalCount =
                queryFactory.select(Projections.constructor(BasicListDto.class, basicEntity))
                        .from(basicEntity)
